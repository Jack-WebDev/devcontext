...
function RunningEnvironmentConflictDialog({
	conflict,
	launchPending,
	error,
	onCancel,
	onLaunchAnother,
}: RunningEnvironmentConflictDialogProps) {
	...
	return (
		<Card
			...
			role="dialog"
			aria-modal="true"
			aria-labelledby="running-environment-conflict-heading"
			className="border border-border py-0"
		>
			<CardContent className="space-y-4 p-5">
				<div>
					<h3
						id="running-environment-conflict-heading"
						className="text-base font-semibold"
					>
						{sameContext
							? "Environment already running"
							: "Project is running in another context"}
					</h3>
					<p className="mt-1 text-sm text-muted-foreground">
						{sameContext
							? "This project is already open with the selected context."
							: "This project is already open with a different context. Contexts stay isolated for the lifetime of each environment."}
					</p>
				</div>
				<dl className="grid gap-2 text-sm">
					<ConflictDetail label="Project" value={environment.project.name} />
					<ConflictDetail
						label="Running context"
						value={environment.context.name}
					/>
					<ConflictDetail label="Coding tool" value={environment.tool.name} />
				</dl>
				{error ? (
				...
				<div className="flex flex-wrap justify-end gap-3">
					...
					>
						{launchPending ? "Launching..." : "Launch another window"}
					</Button>
				</div>
			</CardContent>
		</Card>
