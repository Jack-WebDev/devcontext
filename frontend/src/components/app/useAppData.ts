...
import { type LoadState, loadStateFromResult } from "./load-state";
...
export function useAppData(activeRoute: AppRoute) {
	...
	const [homeDashboard, setHomeDashboard] = useState<
		LoadState<HomeDashboardState>
	>({ status: "loading" });
	const [recentProjects, setRecentProjects] = useState<
		LoadState<RecentProjectState[]>
	>({ status: "loading" });
	const [contexts, setContexts] = useState<LoadState<ContextListItem[]>>({
		status: "loading",
	});
	const [projects, setProjects] = useState<LoadState<ProjectsState>>({
		status: "loading",
	});
	const [history, setHistory] = useState<LoadState<HistoryState>>({
		status: "loading",
	});
	const [running, setRunning] = useState<LoadState<RunningEnvironmentsState>>({
		status: "loading",
	});
	const [settings, setSettings] = useState<LoadState<SettingsState>>({
		status: "loading",
	});
	const [trustCenter, setTrustCenter] = useState<LoadState<TrustCenterState>>({
		status: "loading",
	});

	async function refreshHomeDashboard() {
		setHomeDashboard(
			loadStateFromResult(await devContextApi.getHomeDashboard()),
		);
	}
	...
	async function refreshRecentProjects() {
		...
		setRecentProjects(
			result.ok
				? { status: "loaded", data: result.data.projects }
	...
	async function refreshRunningEnvironments() {
		setRunning(
			loadStateFromResult(await devContextApi.getRunningEnvironments()),
		);
	}
