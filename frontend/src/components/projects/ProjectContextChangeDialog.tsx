...
import type { ContextState, DisplayError, ProjectListItem } from "../../lib/devctx-api";
  ...
  return (
    ...
    >
      <CardContent className="space-y-4 p-5">
        <div>
          <h3 id="project-context-change-title" className="text-base font-semibold">Change project context</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            This changes the development identity Dev Context remembers for {project.project.name}.
          </p>
        </div>

        <dl className="grid gap-2 border-y border-border py-4 text-sm">
          <ProjectContextDetail label="Project" value={project.project.path} />
          <ProjectContextDetail label="Current context" value={project.contextName ?? project.contextId ?? "None"} />
        </dl>

        <label className="grid gap-2 text-sm font-medium" htmlFor="project-context-select">
          New context
          <select
        ...
        {selectedContext ? <ContextSafetySummary context={selectedContext} /> : null}
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="button" disabled={pending || selectedContext === undefined} onClick={() => onConfirm(contextID)}>
            {pending ? "Saving context..." : `Use ${selectedContext?.name ?? "context"}`}
          </Button>
        </div>
      </CardContent>
    </Card>
...
function ContextSafetySummary({context}: {context: ContextState}) {
  ...
  return (
    <section className="space-y-2 border border-border bg-surface-muted p-3" aria-labelledby="project-context-safety-heading">
      <div className="flex items-center justify-between gap-3">
        <h4 id="project-context-safety-heading" className="font-medium">Safety implications</h4>
...
function safetyImplication(status: "ready" | "needs_attention" | "blocked", contextName: string): string {
  switch (status) {
    case "ready":
...
function ProjectContextDetail({label, value}: {label: string; value: string}) {
  return (
