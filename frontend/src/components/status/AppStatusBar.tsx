...
function AppStatusBar({launchState}: {launchState?: LaunchState}) {
  ...
  return <footer className="border-t border-border bg-background px-4 py-2 text-xs text-muted-foreground" aria-label="Application status"><div className="mx-auto flex max-w-6xl flex-wrap gap-x-5 gap-y-1"><span>{isolation}</span><span>System health: {health}</span><span>Warnings: {warningCount}</span><span>Dev Context</span></div></footer>;
}
