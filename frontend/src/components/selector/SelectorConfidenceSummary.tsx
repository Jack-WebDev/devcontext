			...
			<div className="flex min-w-0 items-center justify-between gap-3">
				<div className="min-w-0">
					...
					<p className="mt-1 text-muted-foreground">
						Review the selected development identity before launch.
					</p>
				</div>
				<span className={`shrink-0 font-medium ${status.className}`}>
					{status.label}
				</span>
			</div>
	...
	return (
		<div className="space-y-1 text-sm">
			<div className="flex min-w-0 items-baseline justify-between gap-3">
				<dt className="truncate text-muted-foreground" title={check.label}>
					{check.label}
				</dt>
				<dd className={`shrink-0 font-medium ${status.className}`}>
					{status.label}
				</dd>
			</div>
...
function IsolationConfidenceRow({
	checks,
}: {
	...
	return (
		<div className="space-y-1 text-sm">
			<div className="flex min-w-0 items-baseline justify-between gap-3">
				...
				<dd className={`shrink-0 font-medium ${presentation.className}`}>
					{label}
				</dd>
			</div>
...
function mostSevereConfidenceStatus(
	checks: LaunchConfidenceCheck[],
...
function confidenceStatusPresentation(status: LaunchConfidenceStatus): {
	label: string;
	className: string;
} {
	switch (status) {
...
export { confidenceStatusPresentation, SelectorConfidenceSummary };
