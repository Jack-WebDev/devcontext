...
function ContextCard({
	...
	const unselectedClassName = interactive
	...
	const enabledProviders = context.providers.filter(
...
function ContextHealthSummary({
	...
	const providerChecks = context.confidence.checks.filter(
		(check) => check.component === "provider",
	);
	const toolCheck = context.confidence.checks.find(
		(check) => check.component === "tool" && check.toolId === context.tool.id,
	);
	const isolationChecks = context.confidence.checks.filter(
		(check) => check.component === "isolation",
	);
	const providerStatus = mostSevereStatus(
		providerChecks.map((check) => check.severity),
	);
	const isolationStatus = mostSevereStatus(
		isolationChecks.map((check) => check.severity),
	);
	...
	return (
		<section
			...
			aria-label={`Context health for ${context.name}`}
		>
			<p className="mb-2 text-xs font-semibold tracking-wide text-muted-foreground uppercase">
				Context health
			</p>
...
function HealthSummaryItem({
	...
	return (
		<div className="flex items-center justify-between gap-3">
			...
			<dd className={`font-medium ${presentation.className}`}>
				{presentation.label}
			</dd>
		</div>
	);
...
function mostSevereStatus(
	statuses: LaunchConfidenceStatus[],
): LaunchConfidenceStatus | undefined {
	...
	return (
		<div className="min-w-0 border border-border bg-muted/30 p-3 text-sm">
			<div className="flex min-w-0 items-start justify-between gap-3">
				...
				<Badge
					variant="secondary"
					className={`shrink-0 text-xs font-medium normal-case tracking-normal ${status.className}`}
				>
					{status.label}
				</Badge>
			</div>
			...
			{context.tool.actionHint ? (
				<p className="mt-2 border border-border bg-background p-2 text-xs text-muted-foreground">
					{context.tool.actionHint}
				</p>
			) : null}
		</div>
...
function ProviderIdentityLine({ provider }: { provider: ProviderState }) {
	...
	if (identity.status === "mismatch_evidence" && identity.message) {
		return (
			<p className="mt-1 text-xs text-muted-foreground">{identity.message}</p>
		);
	}
...
function providerSetupGuidance(
	context: ContextState,
	provider: ProviderState,
): string | undefined {
	...
	return (
		provider.actionHint ??
		`${provider.name} is enabled for ${context.name} but is not configured. Open ${context.name} and complete ${provider.name} setup.`
	);
}

function providerStatusPresentation(state: ProviderState["state"]): {
	label: string;
...
function toolStatusPresentation(status: LaunchConfidenceStatus): {
	label: string;
	className: string;
} {
	switch (status) {
...
function contextNavigationDirectionForKey(
	key: string,
): ContextNavigationDirection | undefined {
	if (key === "ArrowDown") {
		return "next";
	}
