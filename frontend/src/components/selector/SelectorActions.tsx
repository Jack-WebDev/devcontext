	...
	return (
		<div className="space-y-4">
			...
			<div className="flex justify-end gap-3">
				...
				>
					{launchPending
						? launchPendingLabel(projectName, contextName)
						: launchLabel}
				</Button>
			</div>
		</div>
...
function LaunchConfidenceFeedback({
	...
	feedback: LaunchConfidenceFeedbackState;
}) {
			...
			{feedback.checks.length > 0 ? (
				<ul className="mt-2 space-y-1 text-xs text-muted-foreground">
					{feedback.checks.map((check) => (
						<li
							key={`${check.component}-${check.providerId ?? check.toolId ?? check.label}`}
						>
							<span className="font-medium text-foreground">
								{check.label}:
							</span>{" "}
							{check.actionHint ?? check.message}
						</li>
					))}
				</ul>
			) : null}
	...
	switch (confidence.status) {
		case "blocked":
			return {
				...
				message:
					"Dev Context cannot confirm a safe launch until these issues are resolved.",
				checks: confidence.checks.filter(
		...
		case "needs_attention":
			return {
				...
				checks: confidence.checks.filter(
					(check) => check.severity === "needs_attention",
				),
			};
		case "ready":
			return {
...
export { launchConfidenceFeedback, SelectorActions };
