...
function OnboardingReplayAction({disabled, onReplay}: {disabled: boolean; onReplay: () => void}) {
  return <div className="mt-6 flex items-start justify-between gap-6 border-t border-border pt-4"><span><span className="block text-sm font-medium">Review onboarding</span><span className="mt-1 block text-sm text-muted-foreground">Show the introduction to development contexts and local isolation again.</span></span><Button type="button" variant="outline" disabled={disabled} onClick={onReplay}>Show onboarding</Button></div>;
}
