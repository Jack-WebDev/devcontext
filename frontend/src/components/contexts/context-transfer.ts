...
// validation.
export function parseContextMetadataExport(value: string): ContextMetadataExport {
  const parsed: unknown = JSON.parse(value);
  if (!isRecord(parsed) || !Number.isInteger(parsed.version) || !isRecord(parsed.context)) {
