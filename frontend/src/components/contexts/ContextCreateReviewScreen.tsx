	...
	return (
		...
		>
			<div className="space-y-2">
				...
				<h2 id="context-review-title" className="text-launcher-title">
					Review your context
				</h2>
				<p className="text-sm text-muted-foreground">
					Confirm what will stay together before creating this isolated
			...
			<ReviewSection title="Identity" onEdit={() => onEdit("identity")}>
				<p className="font-medium">
					{draft.name?.trim() || "Untitled context"}
				</p>
				{draft.purpose ? (
					<p className="text-sm text-muted-foreground">{draft.purpose}</p>
				) : null}
				{draft.description ? (
					<p className="text-sm text-muted-foreground">{draft.description}</p>
				) : null}
				<p className="text-sm text-muted-foreground">
					Appearance: {draft.icon || "Default icon"},{" "}
					{draft.accent || "default accent"}
				</p>
			</ReviewSection>
			...
			<ReviewSection title="Projects" onEdit={() => onEdit("projects")}>
				{projects.length === 0 ? (
					<p className="text-sm text-muted-foreground">
						No projects linked yet. You can add them later.
					</p>
				) : (
			...
			<ReviewSection title="Development tools" onEdit={() => onEdit("tools")}>
				{tools.length === 0 ? (
					<p className="text-sm text-muted-foreground">
						No development tools selected. Dev Context will use the available
						default launch tool; you can configure more tools later.
					</p>
				) : (
					<ul className="space-y-1 text-sm">
						{tools.map((tool) => (
			...
			<ReviewSection title="Isolation and launch behavior">
				<p className="text-sm text-muted-foreground">
					This context keeps its selected tools and their accounts separate.
					When you choose it for a project, Dev Context opens that project with
					this context’s launch tool.
				</p>
			</ReviewSection>
...
function ReviewSection({
	...
	return (
		<section className="rounded-xl bg-muted/35 p-4" aria-label={title}>
